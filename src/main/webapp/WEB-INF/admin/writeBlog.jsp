...
<script type="text/javascript">
	...
	function submitData(){
		...
		if(title==null || title==''){
			...
			$.post("/blog/save",{'title':title,'blogType.id':blogTypeId,'content':content,'contentNoTag':UE.getEditor('editor').getContentTxt(),'summary':UE.getEditor('editor').getContentTxt().substr(0,155),'keyWord':keyWord},function(result){
				if(result.success){
					alert("博客发布成功！");
					resetValue();
				}else{
 ...
 <script type="text/javascript">
    ...
    var ue = UE.getEditor('editor');
			ue.addListener("ready",function(){
				//通过ajax请求数据
				UE.ajax.request("/blogType/selectAll",
						{
							method:"post",
							async : false,
							data:{},
